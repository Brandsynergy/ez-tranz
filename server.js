...
// Generate Gmail-compatible HTML receipt (table-based layout)
    ...
    if (transaction.location.type === 'ip') {
      ...
      locationHtml = `
        <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="margin-top: 24px; padding-top: 24px; border-top: 2px dashed #e5e7eb;">
          <tr>
            <td style="padding: 0;">
              <p style="font-size: 14px; font-weight: 600; color: #374151; margin: 0 0 12px 0; font-family: Arial, sans-serif;">📍 GPS Transaction Location</p>
              <p style="font-size: 12px; color: #6b7280; text-align: center; margin: 8px 0; font-family: Arial, sans-serif;">
                <a href="${linkUrl}" target="_blank" style="color: #6366f1; text-decoration: none; font-family: Arial, sans-serif;">View on Google Maps</a>
              </p>
            </td>
          </tr>
        </table>
...
function generateReceiptHtml(transaction, merchantSettings, isEmail = false) {
  ...
  // If generating for email, use Gmail-compatible table-based layout
  if (isEmail) {
    return generateGmailCompatibleReceiptHtml(transaction, merchantSettings, symbol, date);
  }
